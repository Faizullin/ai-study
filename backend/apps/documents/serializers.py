...
from apps.mlrec.models import TrainType
...
class DocumentListSerializer(TimestampedSerializer):
    ...
    def get_featured_image(self, obj: Document):
        return DocumentFileModelSerializer(obj.featured_image, context=self.context).data if hasattr(obj,
...
class DocumentDetailSerializer(TimestampedSerializer):
    ...
    class Meta:
        model = Document
        fields = ('id', 'status', 'type', 'language', 'title', 'owner', 'featured_image', 'description', 'content',
                  'files', 'subjects', 'course', 'rating_avg', 'rating_count', 'my_rating_value', 'created_at',
                  'updated_at')

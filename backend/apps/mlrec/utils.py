...
from apps.documents.models import Rating, Document
from .models import Status, Export, TrainModelData, TrainType, Suggestion
...
def load_model():
    try:
        model_output_path = Path(BASE_PATH).joinpath("cf-model.pkl")
...
def save_model(algo_data: dict):
    try:
        model_output_path = Path(BASE_PATH).joinpath("cf-model.pkl")
        logger.info(f"save_model: model_output_path = {model_output_path}")
...
def save_cf_pd_dataset(documents: Document):
    try:
        ...
        model_output_path = Path(BASE_PATH).joinpath(f"{export_obj.pk}_{export_obj.train_type}_dataset.csv")
...
class CollaborativeFilteringModel:
    ...
    def train(self, export_obj: Export):
        ...
        try:
            ...
            train_model_obj.users_involved.set(
                UserModel.objects.filter(id__in=df['user_id'].to_list()))
    ...
    def batch_user_predictions(self, start_page=0, offset=250, model=None):
        ...
        user_ids = UserModel.objects.values_list('id', flat=True)  # filter recent
        t1 = time.time()
    ...
    def train_and_batch(self, export_obj: Export):
        ...
        try:
            ...
            for i, value in df.iterrows():
                ...
                recent_suggestions_vals = Suggestion.objects.filter(
                    train_type=TrainType.CONTENT_BASED, object_id=value['id'], content_type=ctype).values("id",
                                                                                                          "document_id",
                                                                                                          "rating_value")
                # created_at__gte=yesterday,
                recent_suggestions_vals_ids: List[int] = [

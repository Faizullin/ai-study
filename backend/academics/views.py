...
class CourseSubscribeView(RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not profile.subscribed_courses.contains(instance):
            return Response({'detail': f"You are not subscribed to course {instance.title}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, *args, **kwargs):
        ...
        if profile.subscribed_courses.contains(instance):
            return Response({'detail': f"You already subscribed to course {instance.title}"}, status=status.HTTP_400_BAD_REQUEST)

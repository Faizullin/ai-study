...
CELERY_BEAT_SCHEDULE = {
    "apps.documents.tasks.update_document_ratings_task": {
        'task': 'apps.documents.tasks.update_document_ratings_task', 'schedule': timedelta(seconds=LEAST_INTERVAL_UNIT),
    },
    'apps.mlrec.tasks.export_dataset_task': {
        'task': 'apps.mlrec.tasks.export_dataset_task', 'schedule': timedelta(seconds=LEAST_INTERVAL_UNIT),
    },
    'apps.mlrec.tasks.train_cf_model_task': {
        'task': 'apps.mlrec.tasks.train_cf_model_task', 'schedule': timedelta(seconds=LEAST_INTERVAL_UNIT * 2),
    },
    'apps.mlrec.tasks.batch_cf_user_prediction_task': {
        'task': 'apps.mlrec.tasks.batch_cf_user_prediction_task',
        'schedule': timedelta(seconds=LEAST_INTERVAL_UNIT * 3),
    },
    'apps.mlrec.tasks.train_cb_model_and_batch_user_prediction_task': {
        'task': 'apps.mlrec.tasks.train_cb_model_and_batch_user_prediction_task',
        'schedule': timedelta(seconds=LEAST_INTERVAL_UNIT * 3),
    },
}

MODELS_DATA_ROOT = str(Path(BASE_DIR).joinpath("mlrec").joinpath("models_data"))
